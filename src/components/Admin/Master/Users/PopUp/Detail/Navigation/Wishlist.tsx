...
import { useGQLWishlist } from "../../../useGQLUser";

type TProps = {
  userId: string;
};

const Wishlist: FC<TProps> = ({ userId }) => {
  const { dispatch } = useContext(AdminNavCtx);
  const { data, error, loading } = useGQLWishlist({ userId });
  return (
    <Main>
          ...
          data?.map((val) => (
            <ItemWrapper
              ...
              onClick={() => {
                dispatch({
                  type: "SHOW_POPUP",
                  value: { name: "BOOK_DETAIL", bookId: val.id, nested: true },
                });
              }}
            >
              <CoverWrapper>
